...
| Plans | Public plans, current plan badge | Coberto | Cypress read-only | Production read-only ok | Alta | `public-plans.cy.ts` e `internal-plan-management.cy.ts` cobrem a leitura: cards renderizados e exatamente um badge de plano atual. |
| Plans | **Troca de plano (ativacao)** | **Nao coberto** | Cypress com `cy.origin` + conta de teste do Stripe | Requer ambiente isolado | Alta | `activatePlan` termina em redirect cross-origin para o Checkout ou para o portal do Stripe — nao ha como assertar isso contra producao. Ate 24/08/2026 a matriz dava este fluxo como coberto por um spec que **nunca podia passar**: ele esperava um `plan-success-message` que a action nao produz. Gap agora declarado, nao disfarcado. |

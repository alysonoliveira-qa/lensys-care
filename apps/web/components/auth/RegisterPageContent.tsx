            ...
            <form onSubmit={handleRegister} className="space-y-4" data-cy={includeTestSelectors ? 'register-form' : undefined}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2 md:col-span-2">
                  <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Nome da Clínica / Consultório *</label>
                  <Input type="text" placeholder="ex: Consultório Clínico de Optometria Dr. Silva" value={clinicName} onChange={(event) => setClinicName(event.target.value)} className="bg-slate-950/50 border-slate-800 text-white placeholder-slate-500 focus:border-violet-500/50 focus:ring-violet-500/20 h-10" required disabled={loading} />
                </div>
                <div className="space-y-2 md:col-span-1">
                  <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Telefone para Contato</label>
                  <div className="relative">
                    <Input type="tel" placeholder="(11) 99999-9999" value={phone} onChange={(event) => setPhone(event.target.value)} className="bg-slate-950/50 border-slate-800 text-white placeholder-slate-500 focus:border-violet-500/50 focus:ring-violet-500/20 h-10 pl-9" disabled={loading} />
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <div className="relative">
                    <Input type="text" placeholder="Seu nome completo" data-cy={includeTestSelectors ? 'register-name-input' : undefined} value={ownerName} onChange={(event) => setOwnerName(event.target.value)} className="bg-slate-950/50 border-slate-800 text-white placeholder-slate-500 focus:border-violet-500/50 focus:ring-violet-500/20 h-10 pl-9" required disabled={loading} />
                    <User className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500" />
                  </div>
                </div>
                <div className="space-y-2 md:col-span-2">
                  <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Como prefere ser chamado?</label>
                  <Input type="text" placeholder="Ex: Dra. Ana ou Ana" value={preferredName} onChange={(event) => setPreferredName(event.target.value)} className="bg-slate-950/50 border-slate-800 text-white placeholder-slate-500 focus:border-violet-500/50 focus:ring-violet-500/20 h-10" maxLength={60} disabled={loading} />
                </div>
                <div className="space-y-2 md:col-span-2">
                  <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">E-mail profissional *</label>
                  <div className="relative">
                    <Input type="email" placeholder="exemplo@clinicamail.com" data-cy={includeTestSelectors ? 'register-email-input' : undefined} value={email} onChange={(event) => setEmail(event.target.value)} className="bg-slate-950/50 border-slate-800 text-white placeholder-slate-500 focus:border-violet-500/50 focus:ring-violet-500/20 h-10 pl-9" required disabled={loading} />
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-500" />
                  </div>
                </div>
                <div className="space-y-2 md:col-span-2">
                  <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">Definir Senha de Acesso *</label>
                  <Input type="password" placeholder="Mínimo 6 caracteres" data-cy={includeTestSelectors ? 'register-password-input' : undefined} value={password} onChange={(event) => setPassword(event.target.value)} className="bg-slate-950/50 border-slate-800 text-white placeholder-slate-500 focus:border-violet-500/50 focus:ring-violet-500/20 h-10" required disabled={loading} />
                </div>
              </div>

              <Button type="submit" variant="premium" className="w-full h-11 text-base font-semibold transition-all duration-200 mt-6" disabled={loading} data-cy={includeTestSelectors ? 'register-submit-button' : undefined}>
                {loading ? (
                  <>
